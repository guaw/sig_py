    ...
    with open(filepath) as f:
        for line in f:
            ...
            if node0id in neighbor:
                neighbor[node0id].append(edgetype)
            else:
                neighbor[node0id] = [edgetype]
            if nodeid in neighbor:
                neighbor[nodeid].append(edgetype)
            else:
                neighbor[nodeid] = [edgetype]
...
def get_sig(nid, nprime):
    ...
    if nid not in neighbor:
        ...
        return d, v
    for j in neighbor_v[nid]:
        neighbor[nid].append(V[j])
...
# python sig2vec.py data/phone/Phone_data data/phone/phone_type_map.txt 0.9 100 1000 output/phone1000/sig_vec_d10.txt output/phone1000/
